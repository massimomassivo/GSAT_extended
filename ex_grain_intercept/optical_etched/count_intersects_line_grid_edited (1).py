...
sys.path.insert(1, r'C:\_Development\ya21864_rep\NIST_Bilderverarbeitung\imppy3d_functions')
...
file_in_path = r"I:\aewk.p\03 Mitarbeiter\Studenten_AEWK\Bergt\06_Data\Liebherr_Analysen\Probe 08 - 005\m-34524 - 2503i12812_segmented.jpg"

# Define the path for the second Excel file to compare results between images
second_excel_path = r"I:\aewk.p\03 Mitarbeiter\Studenten_AEWK\Bergt\06_Data\Liebherr_Analysen\summary.xlsx" #Von Max geändert oder ergänzt
...
# angles.
directory_images_out = r"I:\aewk.p\03 Mitarbeiter\Studenten_AEWK\Bergt\06_Data\Liebherr_Analysen\Bilder mit Schnittpunkten"
...
# Define how many pixels correspond to a certain amount of µm (micrometers) #Von Max geändert oder ergänzt
scalebar_pixel = 464
scalebar_micrometer = 50
pixels_per_micron = scalebar_pixel/scalebar_micrometer  # Example: 10 pixels correspond to 1 µm

# Define the start and end pixels for height and width to cut off scalebar #Von Max geändert oder ergänzt
...
img2temp = img1.copy() #Von Max geändert oder ergänzt
img2=img2temp[start_row:end_row, start_col:end_col] #Von Max geändert oder ergänzt
...
dist_all_arr = []
inv_dist_all_arr = [] #Von Max geändert oder ergänzt
...
for cur_theta in theta_arr:
    ...
    for rr in np.arange(0, n_row2, row_step):
        ...
        for cur_seg in segment_ii:
            ...
            micron_dist = pix_dist / pixels_per_micron  # Convert pixel distance to microns #Von Max geändert oder ergänzt
            dist_all_arr.append([cur_theta, micron_dist])
            inv_dist_all_arr.append([cur_theta, 1 / micron_dist])  # Calculate and store inverse distance in microns #Von Max geändert oder ergänzt
...
inv_dist_all_arr = np.array(inv_dist_all_arr) #Von Max geändert oder ergänzt
...
# -------- SAVE DATA TO CSV FILE -------- (auskommentiert, habe meine eigene Excel erstellt) #Von Max geändert oder ergänzt
...
theta_2d_plt = []
pix_dist_2d_plt = []
inv_pix_dist_2d_plt = []  # Array to store inverse distances #Von Max geändert oder ergänzt
arr_start_i = 0
for m in np.arange(n_segs-1):
    ...
    if np.absolute(theta_mpo - theta_m) >= theta_step_max/2.0:
        ...
        temp_arr_inv = inv_dist_all_arr[arr_start_i:m+1,:] #Von Max geändert oder ergänzt
        theta_2d_plt.append(temp_arr[:,0])
        pix_dist_2d_plt.append(temp_arr[:,1])
        inv_pix_dist_2d_plt.append(temp_arr_inv[:, 1])  # Calculate inverse distances #Von Max geändert oder ergänzt
        arr_start_i = m + 1

temp_arr = dist_all_arr[arr_start_i:n_segs,:]
temp_arr_inv = inv_dist_all_arr[arr_start_i:n_segs,:] #Von Max geändert oder ergänzt
theta_2d_plt.append(temp_arr[:,0])
pix_dist_2d_plt.append(temp_arr[:,1])
inv_pix_dist_2d_plt.append(temp_arr_inv[:, 1])  # Calculate inverse distances #Von Max geändert oder ergänzt
...
ax1.set_ylabel('Segment Length of Grains [µm]') #Von Max geändert oder ergänzt pixels to µm

# Define the input file path and create the output boxplot file path
input_file_path_no_ext, _ = os.path.splitext(file_in_path) #Von Max geändert oder ergänzt
output_boxplot_file = input_file_path_no_ext + "_boxplot.png" #Von Max geändert oder ergänzt

# Speichere den Boxplot
fig1.savefig(output_boxplot_file) #Von Max geändert oder ergänzt
...
for m, cur_theta_arr in enumerate(theta_2d_plt):
    ...
    inv_temp_avg = np.mean(inv_cur_dist_arr)  # Calculate mean of inverse distances #Von Max geändert oder ergänzt
    inv_temp_med = np.median(inv_cur_dist_arr)  # Calculate median of inverse distances #Von Max geändert oder ergänzt
    thickness_avg = 1 / (1.5 * inv_temp_avg)  # Apply thickness formula to mean inverse distance #Von Max geändert oder ergänzt
    thickness_med = 1 / (1.5 * inv_temp_med)  # Apply thickness formula to median inverse distance #Von Max geändert oder ergänzt
    ...
    #print(f"  Average Inverse Grain Size (Pixels): {inv_temp_avg:.6f}")  # Print average inverse grain size #Von Max geändert oder ergänzt
    #print(f"  Median Inverse Grain Size (Pixels): {inv_temp_med:.6f}")   # Print median inverse grain size #Von Max geändert oder ergänzt
    #print(f"  Thickness from Average Inverse Grain Size (Pixels): {thickness_avg:.2f}")  # Print thickness from average inverse distance #Von Max geändert oder ergänzt
    #print(f"  Thickness from Median Inverse Grain Size (Pixels): {thickness_med:.2f}")  # Print thickness from median inverse distance #Von Max geändert oder ergänzt
    ...
    results[cur_theta] = [
        ...
        temp_med,
        temp_std,
        #inv_temp_avg,  # Add average inverse grain size to results #Von Max geändert oder ergänzt
        #inv_temp_med   # Add median inverse grain size to results #Von Max geändert oder ergänzt
        thickness_avg,  # Add thickness from average inverse distance to results #Von Max geändert oder ergänzt
        thickness_med   # Add thickness from median inverse distance to results #Von Max geändert oder ergänzt
    ]
...
inv_temp_avg_all_lines = np.mean(inv_dist_all_arr[:,1])  # Calculate mean of all inverse distances #Von Max geändert oder ergänzt
inv_temp_med_all_lines = np.median(inv_dist_all_arr[:,1])  # Calculate median of all inverse distances #Von Max geändert oder ergänzt
thickness_avg_all_lines = 1 / (1.5 * inv_temp_avg_all_lines)  # Apply thickness formula to mean inverse distance for all lines #Von Max geändert oder ergänzt
thickness_med_all_lines = 1 / (1.5 * inv_temp_med_all_lines)  # Apply thickness formula to median inverse distance for all lines #Von Max geändert oder ergänzt
...
# print(f"  Average Inverse Grain Size (µm): {inv_temp_avg_all_lines:.6f}")  # Print average inverse grain size for all lines #Von Max geändert oder ergänzt
# print(f"  Median Inverse Grain Size (µm): {inv_temp_med_all_lines:.6f}")   # Print median inverse grain size for all lines #Von Max geändert oder ergänzt
print(f"  Thickness from Average Inverse Grain Size (µm): {thickness_avg_all_lines:.2f}")  # Print thickness from average inverse distance for all lines #Von Max geändert oder ergänzt
print(f"  Thickness from Median Inverse Grain Size (µm): {thickness_med_all_lines:.2f}")   # Print thickness from median inverse distance for all lines #Von Max geändert oder ergänzt
